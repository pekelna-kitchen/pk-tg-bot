...
class ViewWarehouse:
    ...
    async def ask(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        for product_id, product_name, product_sym, limit_container, limit_amount in products:
            product_info = ""
            for location_id, location_name, location_symbol in locations:
                instance = util.find_tuple_element(instances, {1: product_id, 2: location_id})
                if not instance:
                    continue
        ...
        #     if user_data[UserDataKey.FIELD_TYPE] == UserDataKey.PRODUCT:
        #         product_str = dbwrapper.find_in_table(
        ...
        #         location_str = dbwrapper.find_in_table(dbwrapper.Tables.LOCATION, 0, user_data[UserDataKey.CURRENT_ID])[1]
